...
def setup_env():
    ...
    Handles Linux/Streamlit Cloud environments robustly.
    """
    lock_file = "playwright_v16_install.lock"
    ...
    # 3. Install System Deps (Soft Fail for Streamlit Cloud)
    print("   [+] Attempting to install system dependencies...")
    try:
        subprocess.run([sys.executable, "-m", "playwright", "install-deps"], check=False)
    except Exception:
        print("   [!] System deps install skipped (likely no sudo access). Continuing...")
...
import threading
import queue
import time  # <--- FIXED: Explicit import
import base64 # <--- FIXED: Explicit import
import aiodns
import ipaddress
import psutil
import traceback
