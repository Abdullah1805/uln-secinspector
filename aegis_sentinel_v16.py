...
# - No runtime pip installs (uses requirements.txt).
# - Robust browser binary installation.
#
# [IDENTITY & COMPLIANCE]
...
try:
    ...
    print(f"CRITICAL ERROR: Missing dependency {e}. Please check requirements.txt")
    sys.exit(1)
...
def setup_env():
    """
    Installs Playwright browsers ONLY. 
    Python packages must be installed via requirements.txt.
    """
    lock_file = "playwright_browser_v16.lock"
    if os.path.exists(lock_file):
        return

    print("⚙️ [Setup] Installing Chromium Browser...")
    try:
        # Install browser binary
        subprocess.run([sys.executable, "-m", "playwright", "install", "chromium"], check=True)
        
        # Create lock file to prevent re-running on every reload
        with open(lock_file, "w") as f:
            f.write("installed")
        print("✅ [Setup] Browser Installed.")
    except subprocess.CalledProcessError as e:
        st.error(f"Browser Install Failed: {e}. Check Streamlit Cloud logs.")
    except Exception as e:
        st.warning(f"Setup Warning: {e}")
...
# ==========================================
# 5. GHOST PROTOCOL (STEALTH ENGINE)
# ==========================================
class GhostEngine:
    """
