...
# =========================================================
# Wayback JSON Discovery (Correct & Fixed)
# =========================================================
def fetch_json_urls(domain):
    try:
        ...
        for row in data[1:]:
            original = row[2]
            if ".json" in original.lower():
                ts = row[1]
                urls.append(f"http://web.archive.org/web/{ts}/{original}")

        return list(set(urls))
    except Exception:
        return []

def extract_policies_multithread(urls, workers=10):
    results = []
...
def build_pdf(domain, findings):
    ...
    if not findings:
        story.append(Paragraph("No high-risk IAM policies were discovered.", styles["Normal"]))
    else:
...
tab1, tab2 = st.tabs(["🧪 Manual Policy Analysis", "🌐 Automated Wayback Hunt"])

# ---------------- Manual ----------------
with tab1:
    policy_text = st.text_area("Paste IAM Policy JSON", height=320)
    if st.button("Analyze IAM Policy"):
        try:
            ...
            if findings:
                ...
                st.success("No critical IAM risks found.")
...
with tab2:
    ...
    if st.button("Start Automated Hunt"):
        if not domain:
            ...
            findings_map = {}
            policies = extract_policies_multithread(urls[:40])

            for url, policy in policies:
                caps = analyze_policy(policy)
                if caps:
                    findings_map[url] = caps
                    st.warning(f"Risky IAM Policy → {url}")
                    for c in caps:
...
st.sidebar.markdown("""
### System Notes
• Wayback CDX API (correct filtering)  
• Multi‑threaded scanning  
• IAM logic‑aware analysis  
