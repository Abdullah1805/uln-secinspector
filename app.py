...
# =========================================================
# Wayback Machine — Pro Cloud Edition
# =========================================================
def fetch_json_urls(domain):
    try:
        # البحث في جميع النطاقات الفرعية
        api = (
            f"https://web.archive.org/cdx/search/cdx"
            f"?url=*.{domain}/*&output=json&collapse=urlkey"
            f"&filter=statuscode:200&limit=500"
        )
        headers = {'User-Agent': 'Mozilla/5.0 SovereignHunter/Pro v3.0'}
        ...
        urls = []
        interesting_extensions = ['.json', '.txt', '.config', '.conf', '.env', '.js']

        for row in data[1:]:
            original = row[2].lower()
            if any(ext in original for ext in interesting_extensions):
                ts = row[1]
...
def extract_policies(urls, workers=10):
    results = []
...
# Streamlit UI
# =========================================================
st.set_page_config("Sovereign IAM Hunter Pro", layout="wide")
st.title("🛡️ Sovereign IAM Hunter Pro — Ultimate Cloud Edition")

tab1, tab2 = st.tabs(["🧪 Manual Policy Analysis", "🌐 Automated Hunting"])
...
with tab2:
    ...
    if st.button("Start Hunting"):
        if not domain:
            ...
            with st.spinner("Querying Wayback Machine for subdomains and multiple file types..."):
                urls = fetch_json_urls(domain)

            st.info(f"Discovered {len(urls)} potentially interesting files.")

            findings_map = {}
            policies = extract_policies(urls[:50])  # limit to avoid overload
...
st.sidebar.markdown("""
### ✔ Cloud-Safe Professional Edition
• Includes subdomains (*.domain.com)  
• Multi-file types (.json, .txt, .env, .conf, .config, .js)  
• HTTPS only, id_ raw fetch  
• Multi-threaded extraction  
• Limit 50 files to avoid timeout  
""")
