...
# =========================================================
# Logic-Aware Analyzer (Detect * and full admin)
# =========================================================
def analyze_policy(policy_json):
    engine = PolicyEngine(policy_json)
    findings = []

    EXTENDED_CAPABILITIES = CAPABILITIES + [
        {"name": "Full IAM Administrative Access", "actions": ["iam:*"], "risk": 10},
        {"name": "Full S3 Administrative Access", "actions": ["s3:*"], "risk": 9},
        {"name": "Dangerous Star Action", "actions": ["*"], "risk": 10}
    ]
...
# =========================================================
# Wayback Machine — Pro Deep Search
# =========================================================
def fetch_json_urls(domain):
    try:
        ...
        headers = {'User-Agent': 'Mozilla/5.0 SovereignHunter/3.0'}
        ...
        # كلمات مفتاحية حساسة وملفات محتملة
        keywords = ['config', 'secret', 'aws', 'iam', 'cred', 'env', 'setup']
        for row in data[1:]:
            url_path = row[2].lower()
            if ".json" in url_path or any(key in url_path for key in keywords):
                ts = row[1]
    ...
    if not findings:
        ...
        for url, caps in findings.items():
            ...
            for c in caps:
                story.append(
                    Paragraph(
                        f"• <b>{c['capability']}</b> — Risk {c['risk']}/10<br/>Actions: {', '.join(c['actions'])}",
                        styles["Normal"]
                    )
                )
    doc.build(story)
...
st.title("🛡️ Sovereign IAM Hunter Pro — Deep Cloud Edition")

tab1, tab2 = st.tabs(["🧪 Manual Policy Analysis", "🌐 Automated Deep Hunting"])
...
with tab2:
    ...
    if st.button("Start Deep Hunt"):
        if not domain:
            st.error("Please enter a domain.")
        else:
            with st.spinner("Querying Wayback Machine deeply for subdomains and secrets..."):
            ...
            for url, policy in policies:
                ...
                if caps:
                    ...
                    for c in caps:
                        st.write(f"• {c['capability']} (Risk {c['risk']}/10)")
...
• Includes all subdomains (*.domain.com)  
• Multi-file types & keywords (.json, .txt, .conf, .config, .env, .js, secret, iam, aws)  
• Detects wildcard '*' in Action/Resource (Full Admin)  
• Multi-threaded extraction  
• Raw JSON fetch (id_)  
• Limit 60 files to avoid timeout  
""")
