...
# =========================================================
# Capability Database (Defensive Only)
# =========================================================
CAPABILITIES = [
    {
...
# =========================================================
# Recon Engine (Wayback Machine - Public Data Only)
# =========================================================
def fetch_json_urls(domain):
    try:
        ...
        return [u for u in urls if u.endswith(".json")]
    except Exception as e:
        st.error(f"Wayback error: {e}")
        return []
...
# Streamlit UI
# =========================================================
st.set_page_config(page_title="Sovereign IAM Hunter", layout="wide")
st.title("🛡️ Sovereign IAM Hunter — Unified Edition")
...
# ---------------------------------------------------------
with tab_auto:
    st.subheader("Public Archive IAM Hunting (Wayback)")
    ...
    if st.button("Start Hunting"):
        if not domain:
            ...
            total_findings = 0
            for url in urls[:40]:  # limit to avoid abuse
...
st.sidebar.markdown("""
### 🧠 About
- Public data only (Wayback Machine)
